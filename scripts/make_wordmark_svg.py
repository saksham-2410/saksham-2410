...
CELL_W = 9.0
CELL_H = 15.5
# Avenir Next Bold: even stroke weight keeps the shading consistent across a
# letter (Futura itself isn't bundled on this Mac). a very heavy face (Impact)
# collapses to blobs at this resolution -- the counters get thinner than one
# grid cell.
...
# two letters across the full width leaves more grid columns each than Avi's
# three-letter "AVI" -- if it renders too chunky, TRACKING/DEPTH_FRAC below are
# the knobs to widen the gaps back out.
TEXT = os.environ.get("WORDMARK_TEXT", "SD")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">sd@github: ~$ ./wordmark.sh --3d</text>')
