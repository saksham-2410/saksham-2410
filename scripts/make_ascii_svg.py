...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "sd-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">sd@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'sd@github:~$ whoami <tspan fill="{INK}">Saksham Dewan</tspan></text>')
